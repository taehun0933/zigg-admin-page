...
import { getFileExtension, formatHms, getVideoDurationSec } from "@/utils/media";
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="text-sm font-medium">이미지 ({totalImageCount}/{MAX_FILES})</div>
                <div className="space-x-2">
                  <input
                    id="imageUpload"
                    type="file"
                  ...
                  <label
                    ...
                    className={`inline-block px-3 py-1.5 rounded-md text-white text-sm cursor-pointer ${
                      totalImageCount >= MAX_FILES ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
                    }`}
                  >
                    이미지 추가
                  </label>
              ...
              {keptImages.length > 0 && (
                <div>
                  <div className="text-xs text-gray-500 mb-1">현재 유지하는 이미지</div>
                  <div className="grid grid-cols-3 gap-3">
                    {keptImages.map((u, i) => (
                      <div key={`${u}-${i}`} className="relative">
                        <img src={u} className="w-full h-28 object-cover rounded-md border" />
                        <button
                          type="button"
              ...
              {newImages.length > 0 && (
                <div>
                  <div className="text-xs text-gray-500 mb-1">추가된 새 이미지</div>
                  <div className="grid grid-cols-3 gap-3">
                    {newPreviews.map((u, i) => (
            ...
            {newVideoPreview && (
                ...
                <video src={newVideoPreview} controls className="w-full rounded-md border" />
                <button
                    type="button"
                ...
                <div className="text-xs text-gray-500 mt-1">{newVideo?.name}</div>
                </div>
            )}
            </div>
