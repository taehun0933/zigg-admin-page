...
import { getUrlForUploadImage, putImageToPresignedUrl,requestImagePresignedUrl, requestVideoPresignedUrl } from "@/apis/media";
...
export default function NoticeCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (bannerImageFile) {
          const { url,contentId } = await requestImagePresignedUrl(bannerImageFile, "NOTICE_BANNER");
      ...
      <main className="max-w-4xl mx-auto p-4 pt-12">
        <div className="bg-white rounded-lg shadow p-6 space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="font-medium ml-6">노출 우선순위(높을수록 우선입니다)</label>
            <input
              type="number"
              value={priority}
        ...
        <div className="flex items-center justify-between">
            ...
            {bannerImageFile && (
                <>
                <span className="truncate max-w-[180px] sm:max-w-[280px]">{bannerImageFile.name}</span>
                <button
                    type="button"
        ...
        <div className="flex items-center gap-2">
            ...
            >
            <svg width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="M19 15v4H5v-4H3v4a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-4zM11 7.83V17h2V7.83l3.59 3.58L18 10l-6-6l-6 6l1.41 1.41z"/></svg>
            이미지 선택
            </label>
            <input
  id="bannerUpload"
  ...
  onChange={async (e) => {
    ...
    try {
      ...
      if (Math.abs(currentAspect - BANNER_ASPECT) > ASPECT_EPS) {
        setBannerNotice(
          `현재 이미지 비율은 ${currentRatioStr} (≈ ${(currentAspect).toFixed(3)}) 입니다. ` +
          `권장 9:5.16(≈ ${(BANNER_ASPECT).toFixed(3)}) 비율에 맞춰 자동 조정되어 업로드됩니다.`
        );
      } else {
    ...
    {bannerNotice}
        ...
        <div className="aspect-[900/516] w-full overflow-hidden rounded-xl ring-1 ring-gray-200 bg-gradient-to-b from-gray-50 to-white">
            ...
            <img
                src={bannerPreview}
                alt="banner"
                className={`w-full h-full ${bannerFit === "cover" ? "object-cover" : "object-contain"} bg-white`}
            />
            ) : (
